...
public:
    ...
    std::vector<int> leaves() {
        ...
        bypass_leaves(root_, &result);
        return result;
    }

private:

    void bypass_leaves(Node* root, std::vector<int>* result) {
        ...
        if (root->left != nullptr) {
            bypass_leaves(root->left, result);
        }

        if (root->right != nullptr) {
            bypass_leaves(root->right, result);
        }
    }
...
int main() {
    ...
    std::vector<int> result = bst.leaves();
    for (auto i : result) {
        std::cout << i << std::endl;
    }
}